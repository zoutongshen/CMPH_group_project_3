...
## 8b. Same gravity, two algorithms (NEW — tree vs direct N²)

⏱ 0:45 (cum 7:05). Both codes integrate the same softened Newtonian force —
they differ in how the sum over particles is evaluated. Direct N² computes
every pair exactly, O(N²). The Barnes–Hut octree recursively splits space
into octants and replaces a distant cell by its centre of mass whenever the
cell's size over its distance is below the opening angle θ = 0.5 — that
collapses the cost to O(N log N) at the price of a small, controlled force
error (median 0.4% per evaluation here). The figure is the cross-validation:
the SAME equilibrium galaxy (manual ICs), same leapfrog, same Δt = 0.125 and
ε = 0.1, evolved to t = 100 by both algorithms — the disk surface-density
profile and rotation curve lie on top of each other. So the two
implementations validate each other, and the 0.4% force error is dynamically
irrelevant. Cost side (backup figure): at N = 80k one force evaluation is
1.01 s direct vs 0.15 s tree — the tree wins asymptotically, which is why
production galaxy-formation codes are tree-based. If asked why we still used
direct N²: at N ≲ 10⁵ the JIT-parallel direct sum is competitive (0.06 s vs
0.04 s at N = 20k), it's exact, and it conserves momentum to machine
precision (the tree's cell approximation breaks pair symmetry at the error
level). Generated by galaxy_merger_octree/method_comparison.py.
